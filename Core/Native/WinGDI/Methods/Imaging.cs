...
namespace Ax.Engine.Core.Native
{
    public static partial class WinGdi
    {
        ...
        /// <param name="rop">A raster-operation code. These codes define how the color data for the source rectangle is to be combined with the color data for the destination rectangle to achieve the final color. See <see cref="BITBLT_OP"/>.</param>
        /// <returns>If the function succeeds, returns TRUE, otherwise, retun FALSE.</returns>
        [DllImport("gdi32.dll")] public static extern bool BitBlt([In] IntPtr hdc, [In] int x, [In] int y, [In] int cx, [In] int cy, [In] IntPtr hdcSrc, [In] int x1, [In] int y1, [In] int rop);
        ...
        [DllImport("gdi32.dll")] public static extern IntPtr CreateCompatibleBitmap([In] IntPtr hdc, [In] int cx, [In] int cy);
    }
}
